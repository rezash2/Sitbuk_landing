...
## Stage 63 - فرم مشاوره بالای محصولات و حذف برچسب ویدیوهای صفحه اصلی
- ادامه از Stage 62 بدون rollback انجام شد.
- برچسب‌های روی ویدیوهای صفحه اصلی، شامل «ویدیو معرفی» و شماره/عنوان ویدیوهای کوتاه، از روی پلیرها حذف شدند تا کاورها تمیزتر نمایش داده شوند.
- یک فرم مشاوره رایگان با طراحی تیره و هماهنگ با تم سیت‌باک، درست قبل از بخش «محصولات سیت‌باک» در صفحه اصلی اضافه شد.
- فرم جدید نام، شماره تماس، نام شرکت و نوع کسب‌وکار را دریافت می‌کند و با همان مسیر امن `submit_lead` و Ajax قبلی ثبت می‌شود.
- نوع کسب‌وکار بدون migration جدید در توضیحات لید ذخیره می‌شود تا با دیتابیس‌های فعلی سازگار بماند.
- CSS فقط در فایل مفهومی موجود `landing/static/landing/css/style.css` اضافه شد و فایل مرحله‌ای جدید ساخته نشد.

## Stage 64 - نمایش دو ستونه کارت‌های محصولات سیت‌باک در موبایل
- کارت‌های بخش «محصولات سیت‌باک» در صفحه اصلی برای عرض موبایل به صورت دو کارت کنار هم تنظیم شد.
- چیدمان دسکتاپ بدون تغییر باقی ماند و فقط media query موبایل اصلاح شد.
- ابعاد، تایپوگرافی و پشت کارت‌ها در موبایل فشرده‌سازی شد تا در چیدمان دو ستونه از کادر خارج نشوند.
- فایل CSS مرحله‌ای جدید ساخته نشد و اصلاحات داخل `landing/static/landing/css/style.css` اعمال شد.

## Stage 65 - هماهنگ‌سازی فونت دراپ‌داون فرم مشاوره صفحه اصلی
- فونت فیلد انتخاب «نوع کسب‌وکار» در فرم مشاوره بالای محصولات با فونت اصلی سایت `IRANSansX` هماهنگ شد.
- برای متن گزینه‌های داخل select نیز خانواده فونت، وزن و fallbackهای سازگار با سایت تنظیم شد.
- تغییر فقط در CSS مفهومی فعلی `style.css` اعمال شد و هیچ فایل مرحله‌ای جدید یا migration جدید اضافه نشد.
